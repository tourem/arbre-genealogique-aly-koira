...
| `izé` (variantes : `izô`, `izé woy`, `iza arou`) | ♂ ou ♀ | Enfant, ou enfant d'un frère-équivalent de même sexe (pour un homme) ou d'une sœur-équivalente de même sexe (pour une femme) |
...
| `baassa arou` | ♂ | Cousin croisé homme (par parents de sexes opposés) |
...
| `kaga arou` | ♂ | Grand-père, ou tout ancêtre mâle indirect au-delà de 2 générations |
...
- `kaga arou` = grand-père
- `kaga kaga arou` = arrière-grand-père
- `kaga kaga kaga arou` = arrière-arrière-grand-père
- ... et ainsi de suite

Pour préciser la branche :
- `kaga arou coté baba` = grand-père paternel
- `kaga arou coté gna` = grand-père maternel
...
- `izé woy` ou `iza arou` : forme spécifique (au choix de la convention applicative)
...
| **dA = 0, dB ≥ 2** : A ancêtre éloigné de B | A♂ → `kaga arou`, A♀ → `kaga woy` (répété `dB-1` fois), suffixé par `coté baba` ou `coté gna` selon le 1er saut du chemin de B | B reçoit `haama` (répété `dB-1` fois) |
| **dA = dB ≥ 1**, parents directs PA et PB de **même sexe** | — | A et B sont `arma` (♂) ou `woyma` (♀) mutuellement |
| **dA = dB ≥ 1**, parents directs PA et PB de **sexes opposés** | — | A et B sont `baassa arou` (♂) ou `baassa woy` (♀) mutuellement |
...
| **dA + k = dB** avec **k ≥ 2** : A grand-oncle, etc. | A♂ → `kaga arou` (×(k-1)), A♀ → `kaga woy` (×(k-1)), suffixé `coté baba` ou `coté gna` | B reçoit `haama` (×(k-1)) |

### Détermination du "côté baba/gna"

Pour les termes `kaga arou` et `kaga woy` qui nécessitent une précision sur la branche :
...
type Relation = {
  termForA: string;           // ex: "hassa", "kaga arou coté baba"
...
function classer_descendance(ancetre, descendant, distance, pathDescendant):
    ...
    sinon:  // distance >= 2
        nb_kaga = distance - 1
        kaga_word = "kaga " * nb_kaga + ((sexe_ancetre == 'M') ? "arou" : "woy")
...
function classer_meme_generation(A, B, instance):
    ...
    sinon:
        // Cousins croisés
        terme_pour_A = (A.sex == 'M') ? "baassa arou" : "baassa woy"
        terme_pour_B = (B.sex == 'M') ? "baassa arou" : "baassa woy"
...
function classer_decalage(A, B, instance):
    ...
    sinon:  // delta >= 2 : grand-oncle, arrière-grand-oncle, etc.
        ...
        nb_kaga = delta - 1
        kaga_word = "kaga " * nb_kaga + ((sup.sex == 'M') ? "arou" : "woy")
...
Soit l'arbre suivant (à utiliser pour validation, 28 personnes sur 7 générations) :
...
```
                ...
                (♂)    (♂) (♀)      (♂)    (♂)   (♀)
                       /  |  \    \              |     \
                  Bakary Adama Djéneba Khadidia Koniba  Mariam
                  (♂)   (♂)   (♀, G3)  (♀, G3) (♂, G3) (♀, G3)
                   |          (mariée            |
...
Les personnes ajoutées par rapport à la première version sont :
- **Khadidia** (♀, G3, fille d'Awa) — pour illustrer `gna` en tante parallèle ♀♀ et `baassa woy` en cousine croisée
- **Mariam** (♀, G3, fille de Niamoye) — pour illustrer `woyma` en cousine parallèle ♀♀ après reset baassa
...
| Modibo ↔ Lassana | dist=3 | (Modibo) `kaga kaga arou coté baba` ↔ (Lassana) `haama haama` (arrière-grand-père) |
...
### Cas test 12 — Khadidia (gna en tante parallèle)

**Khadidia ↔ Niamoye** :
LCA = Sira. Khadidia → Awa(M) → Modibo(P) → Sira(M), donc dA=3, pathA=['M','P','M']. Niamoye → Hadja(M) → Sira(M), donc dB=2, pathB=['M','M']. delta=1, donc avuncular.
Couple sibling-équivalent : (Niamoye, parent_de_Khadidia_au_même_niveau) = (Niamoye♀, Awa♀). Mêmes sexes → parallèle.
Niamoye♀ → terme_sup = `gna`. Khadidia → terme_inf = `izé`.
**Résultat** : Niamoye est `gna` pour Khadidia. Khadidia est `izé` pour Niamoye.

C'est le cas illustratif du `gna` en tante parallèle : Niamoye n'est pas la mère biologique de Khadidia mais en porte le titre car elle est sœur-équivalente ♀ de la mère Awa.

### Cas test 13 — Khadidia ↔ Mariam (woyma cousines parallèles après reset)

**Khadidia ↔ Mariam** : Khadidia est fille d'Awa, Mariam est fille de Niamoye. Awa et Niamoye sont elles-mêmes baassa (parents Modibo♂ et Hadja♀, cross). Mais leurs filles : parents directs Awa♀ et Niamoye♀, mêmes sexes → parallèle.
**Résultat** : Mariam est `woyma` pour Khadidia, Khadidia est `woyma` pour Mariam — comme deux sœurs.

C'est le cas le plus pédagogique du **reset baassa** au niveau cousin germain : malgré que leurs mères soient cousines croisées, les filles sont nommées comme des sœurs.

### Cas test 14 — Khadidia ↔ Djéneba (baassa woy entre cousines croisées)

LCA = Modibo (G1). Khadidia → Awa(M) → Modibo(P), dA=2. Djéneba → Sékou(P) → Modibo(P), dB=2.
Parents directs : Awa♀ pour Khadidia, Sékou♂ pour Djéneba. Sexes opposés → cross → baassa.
Khadidia♀ → `baassa woy`. Djéneba♀ → `baassa woy`.
**Résultat** : `baassa woy` réciproque.

C'est l'exemple direct au niveau cousin germain de `baassa woy`.
