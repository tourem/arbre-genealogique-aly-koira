...
| code | term_songhoy | label_fr | sex_of_speaker | sex_of_target | context_condition |
|------|-------------|----------|----------------|---------------|-------------------|
| `KAAGA` | KAAGA | Grand-père (niveau 1) | ANY | M | LEVEL_1 |
| `KAAGA_WOY` | KAAGA WOY | Grand-mère (niveau 1) | ANY | F | LEVEL_1 |
| `KAAGA_BERI_DJINA` | KAAGA BERI DJINA | Ancêtre homme niveau 2 | ANY | M | LEVEL_2 |
| `KAAGA_WOY_BERI_DJINA` | KAAGA WOY BERI DJINA | Ancêtre femme niveau 2 | ANY | F | LEVEL_2 |
| `KAAGA_BERI_HINKATO` | KAAGA BERI HINKATO | Ancêtre homme niveau 3 | ANY | M | LEVEL_3 |
| `KAAGA_WOY_BERI_HINKATO` | KAAGA WOY BERI HINKATO | Ancêtre femme niveau 3 | ANY | F | LEVEL_3 |
| `KAAGA_BERI_HINZANTO` | KAAGA BERI HINZANTO | Ancêtre homme niveau 4 | ANY | M | LEVEL_4 |
| `KAAGA_WOY_BERI_HINZANTO` | KAAGA WOY BERI HINZANTO | Ancêtre femme niveau 4 | ANY | F | LEVEL_4 |
| `KAAGA_BERI_TAATCHANTO` | KAAGA BERI TAATCHANTO | Ancêtre homme niveau 5 | ANY | M | LEVEL_5 |
| `KAAGA_WOY_BERI_TAATCHANTO` | KAAGA WOY BERI TAATCHANTO | Ancêtre femme niveau 5 | ANY | F | LEVEL_5 |
| `KAAGA_BERI_GOUWANTO` | KAAGA BERI GOUWANTO | Ancêtre homme niveau 6 | ANY | M | LEVEL_6 |
| `KAAGA_WOY_BERI_GOUWANTO` | KAAGA WOY BERI GOUWANTO | Ancêtre femme niveau 6 | ANY | F | LEVEL_6 |

> Pour les niveaux ≥ 7 : le code suit le pattern `KAAGA_BERI_{N}` / `KAAGA_WOY_BERI_{N}`,
> le terme Songhoy est **KAAGA BERI** + N / **KAAGA WOY BERI** + N.
> L'admin peut ajouter de nouveaux niveaux si nécessaire.

**Correspondance niveau ↔ diff :**

| Niveau KAAGA | diff (distance générations) | Suffixe Songhoy |
|:---:|:---:|:---|
| 1 | 2 | *(pas de suffixe)* |
| 2 | 3 | BERI DJINA |
| 3 | 4 | BERI HINKATO |
    ...
    POUR CHAQUE ancestorId DANS communsIds:
        ...
        SINON:
            ...
            SINON:
                ...
                // Construire le code du terme selon le sexe et le niveau
                prefixe ← (personA.sex = M) ? "KAAGA" : "KAAGA_WOY"
                
                SI niveauKaaga = 1:
                    codeTerme ← prefixe                          // KAAGA ou KAAGA_WOY
                SINON SI niveauKaaga = 2:
                    codeTerme ← prefixe + "_BERI_DJINA"          // KAAGA_BERI_DJINA
                SINON SI niveauKaaga = 3:
                    codeTerme ← prefixe + "_BERI_HINKATO"        // KAAGA_BERI_HINKATO
                SINON SI niveauKaaga = 4:
                    codeTerme ← prefixe + "_BERI_HINZANTO"       // KAAGA_BERI_HINZANTO
                SINON SI niveauKaaga = 5:
                    codeTerme ← prefixe + "_BERI_TAATCHANTO"     // KAAGA_BERI_TAATCHANTO
                SINON SI niveauKaaga = 6:
                    codeTerme ← prefixe + "_BERI_GOUWANTO"       // KAAGA_BERI_GOUWANTO
                SINON:
                    codeTerme ← prefixe + "_BERI_" + niveauKaaga // KAAGA_BERI_7, etc.
                
                // Lookup dans la table de référence
                SI terms[codeTerme] EXISTE:
                    relation.termAtoB ← terms[codeTerme]
                SINON:
                    // Fallback : construire dynamiquement
                    relation.termAtoB ← {
                        songhoy: (personA.sex = M ? "KAAGA BERI " : "KAAGA WOY BERI ") + niveauKaaga,
                        french: "Ancêtre de niveau " + niveauKaaga
                    }
...
Pour le cas KAAGA, afficher le terme Songhoy complet :

```
│  📌 KAAGA (Grand-parent / Ancêtre)                       │
│                                                          │
│  👤 Omar → Seydou :                                      │
│     Omar est KAAGA BERI DJINA pour Seydou                │
...
### Test 12 — Grand-parent niveau 1 (KAAGA)
```
Ancêtre commun = G
G → A (fils, M)                   ← niveau 1
G → X (autre enfant) → Y → B     ← niveau 3
diff = 2, niveauKaaga = 1
→ Attendu : GRANDPARENT, A=KAAGA pour B, B=HAAMA pour A
```

### Test 13 — Grand-parent niveau 2, femme (KAAGA WOY BERI DJINA)
```
Ancêtre commun = G
G → A (fille, F)                       ← niveau 1
G → X → Y → Z → B                     ← niveau 4
diff = 3, niveauKaaga = 2
→ Attendu : GRANDPARENT, A=KAAGA WOY BERI DJINA pour B, B=HAAMA pour A
```

### Test 13b — Grand-parent niveau 3, homme (KAAGA BERI HINKATO)
```
Ancêtre commun = G
G → A (fils, M)                        ← niveau 1
G → X → Y → Z → W → B                 ← niveau 5
diff = 4, niveauKaaga = 3
→ Attendu : GRANDPARENT, A=KAAGA BERI HINKATO pour B, B=HAAMA pour A
```
...
| KAAGA | kaa-ga | Grand-père / ancêtre homme (niveau 1) |
| KAAGA WOY | kaa-ga ouoy | Grand-mère / ancêtre femme (niveau 1) |
| KAAGA BERI DJINA | kaa-ga bé-ri dji-na | Ancêtre homme niveau 2 |
| KAAGA BERI HINKATO | kaa-ga bé-ri hin-ka-to | Ancêtre homme niveau 3 |
| KAAGA BERI HINZANTO | kaa-ga bé-ri hin-zan-to | Ancêtre homme niveau 4 |
| KAAGA BERI TAATCHANTO | kaa-ga bé-ri taat-chan-to | Ancêtre homme niveau 5 |
| KAAGA BERI GOUWANTO | kaa-ga bé-ri gou-wan-to | Ancêtre homme niveau 6 |
| HAAMA | haa-ma | Petit-enfant / descendant |

> 💡 Pour les termes féminins, ajouter **WOY** après KAAGA : KAAGA WOY BERI DJINA, etc.
> **BERI** = grand. Suffixes de comptage : DJINA (1er), HINKATO (2ème), HINZANTO (3ème), TAATCHANTO (4ème), GOUWANTO (5ème).
